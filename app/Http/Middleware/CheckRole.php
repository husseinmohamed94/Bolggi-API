...
class CheckRole
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if(auth()->check()){
           ...
           $routeDisitiation = in_array($route[0],$roleRoutes) ? $route[0]. '.show_login_form' : 'frontend.show_login_form';
            $path = $route[0] != ''  ?   $routeDisitiation : auth()->user()->roles[0]->allowed_route.'.index';
